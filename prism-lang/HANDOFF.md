...
- **ドキュメントに載せる静的チェッカーの出力・数値は、必ず実機から取って貼る**（推測で書かない＝過去に陳腐化/捏造を繰り返した教訓）。
...
関数シグネチャ = **「正直な契約」**：`f(x: A) : B !world ?Error <- ...`。**AIが本体を書き、人間が契約を読み、静的チェッカーが契約を守らせる。**
契約が保証するのは**「何に触れ・どう失敗しうるか」の輪郭**であって、**計算の正しさそのものではない**（README/AI_GUIDE の表現はこの線で正直に書いてある）。

**最重要の結論**: Prism の価値は新奇な構文ではなく**型システム（静的チェッカー）**にある。静的チェッカーなしの記法はただの飾り。
...
- **`check.py`** — 静的チェッカー。式を **(型, 効果集合, 失敗集合)** に合成。型単一化＋行多相＋網羅性＋能力＋高階種＋時間＋補間＋**レコードのフィールド型**。`--reveal` で契約推論。
...
### ドキュメント (.md)
- `README.md` — **短い案内板**（3デモ hook ＋ 保証/非保証表 ＋ 索引）。`OVERVIEW.md` — 思想と使い方。`GETTING_STARTED.md` — 導入。`TUTORIAL.md` — 6課。`ALGORITHMS.md` — アルゴリズム＋全ギャラリー目録（絵・アニメ）。`REFERENCE.md` — 完全仕様（文法EBNF・エラーカタログ・描画/アニメ契約）。**`AI_GUIDE.md`** — LLM に貼る最小仕様＋禁止事項＋実演。**`NOTES.md`** — 設計日誌（静的チェッカーの思想・18の発見・ロードマップ）。`LICENSE`(MIT)。
...
- **能力(capability)**: `capability Ord for T`＝トレイト, `Num provides Ord`＝インスタンス。静的チェッカーは (1)制約 discharge (2)インスタンス完全性 (3)coherence(型×能力で1つ) (4)メソッド本体の契約。実行時は第1引数の型タグで単一ディスパッチ。
...
- **グラフィクス（Layer A / B、ほぼ言語変更ゼロで実現）**:
  ...
  - **アニメ** = 純粋関数 `frame(t) : Picture`。playground の ▶Animate が `t`(秒)を進めて毎フレーム再描画。**`!` が無い＝静的チェッカーがアニメの純粋性を保証**（裏で `show!console` すると弾かれる）。`frac`/`mod`（＋組み込み `floor`）で寿命をループ。
